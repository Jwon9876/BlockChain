...
class Blockchain:
    ...
    def chain_is_valid(self, chain):
            ...
            for i in range(1, len(chain)):
                ...
                # if #채우시오 : i가 1이 아니거나 현재 블럭의 previous_hash값과 이전 블럭의 hash값이 같은지 비교:
                if i != 1 or prev_block['hash'] == current_block['previous_block_hash']:
                    validChain = False
        ...
        if len(transactions) == 0:
            ...
            while len(transactions) > 1:
                ...
                if len(transactions) % 2 == 0:
                    #채우시오 : 가장 마지막 tx를 trasactions에 추가 append (- 1)
